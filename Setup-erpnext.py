...
def install_bench(args):
	...
	if find_executable('pip'):
		run_os_command({
			'pip': 'pip install --upgrade setuptools urllib3 cryptography pip'
		})
		...
		if success:
			run_os_command({
				'pip': 'pip install --upgrade setuptools urllib3 requests cryptography pip'
			})

	success = run_os_command({
		'pip': "pip install --upgrade urllib3 cryptography ansible"
	})
	...
	if args.user == 'root':
		raise Exception('Please run this script as a non-root user with privileges, but without using or pass --user=USER')
